...
class BuroApiProvider {
 ...
 Future<Leaveinfo> getLeaveRecord() async {
    var user = await sessionManager.userID;
    var password = await sessionManager.password;
    var token = await sessionManager.token;
...
//The below code is responsible to fetch district by division
  ...
  Future<dynamic> getDivisionList() async {
    ...
    var response =
        await http.get(Uri.parse(environments.base_url + environments.my_leave_getdivision), headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token',
    });
    ...
    // try {
    //   response = await networkConfigWithToken('$token')
    //       .get(environments.my_leave_getdivision);
  ...
  //   try {
  //     final response = await networkConfigWithToken('$token').get(
  //       environments.my_leave_getdivision,
  // );
  ...
  Future<dynamic> getDistricttListByDivision(var divisionlink) async {
    var token = await sessionManager.token;

    var divlink = "/" + divisionlink;
    var response = await http.get(
        Uri.parse(environments.base_url +
    ...
    // try {
    //   final response = await networkConfigWithToken('$token').get(
    //     environments.my_leave_getdistrict_bydivision + divlink,
    //   );
    ...
    //     print('Authentication failed error');
    //     await getToken(user, password);
    //     return getDistricttListByDivision(divisionlink);
    //   }
    //   throw DioException.fromDioError(e);
    // }
...
//The below code is responsible for fetching thana by providing District
  Future<dynamic> getThanaListbydistrict(var district) async {
    var token = await sessionManager.token;
    var districtname = "/" + district;
    var response = await http.get(
        Uri.parse(environments.base_url +
            environments.my_leave_getthanaby_district +
            districtname),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
    ...
    // try {
    //   final response = await networkConfigWithToken('$token').get(
    //     environments.my_leave_getthanaby_district + districtname,
    //   );
